...
/** functions */ function onFormSubmit(evt) {
    evt.preventDefault();
    clearTimeout(timeId);
    const { delay: delay1 , step , amount  } = evt.target.elements;
    let stepValue = Number(delay1.value);
    if (delay1.value < 1 || step.value < 1 || amount.value < 1) {
        (0, _notiflix.Notify).failure(`All fields must be more than zero`);
        return;
    }
    (0, _notiflix.Notify).success("Please observe the promises appear below");
    for(let i = 1; i <= amount.value; i += 1){
        createPromise(i, stepValue).then(({ position , delay  })=>{
